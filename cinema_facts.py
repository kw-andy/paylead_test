...
def choose_cols_theaters(csv_file, networks):
    ...
    for idx_seats, seats in enumerate(df["capacity"]):
        theater_dict[idx_seats].append(seats)
    ...
    for keys_theaters, vals_theaters in theater_dict.items():
        for keys_networks, vals_networks in networks.items():
            if vals_networks in vals_theaters:
                theater_dict[keys_theaters][vals_theaters.index(vals_networks)] = str(
                    keys_networks
                )
    return theater_dict
...
def show_me_min_max_stats(min_max_input, dbfile):
    ...
    if val_switch == "min":
        ...
        print("The theater with the smallest number of seats is the following:")
        print(cur.fetchall())
    elif val_switch == "max":
        cur.execute(
            "SELECT osm_id,com_nom,MAX(seats) FROM theaters WHERE seats IS NOT NULL GROUP BY seats ORDER BY seats DESC limit 1"
        )
        print("The theater with the largest number of seats is the following:")
        print(cur.fetchall())
    con.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs=3,
        type=str,
        help="Required: url zip_filename csv_filename example: 'https://www.data.gouv.fr/fr/datasets/r/1d8c2bc0-d769-433a-b8ab-0f49341d4b8a' 'movie.zip' 'data.csv'",
    )
    parser.add_argument(
        "--transform", type=str, nargs=2, help="Required: csv_filename db_filename example 'data.csv' 'paylead.db'"
    )
    parser.add_argument(
        "--stats", type=str, nargs=2, help="Required: csv_filename db_filename example 'min' 'paylead.db'"
    )
    args = parser.parse_args()
    if args.download:
        url, zip_filename, csv_filename = args.download
        down_and_unzip(url, zip_filename, csv_filename)
    elif args.transform:
        csv_filename, db_filename = args.transform
        transform_and_load_db(csv_filename, db_filename)
    elif args.stats:
        csv_filename, db_filename = args.stats
        show_me_min_max_stats(csv_filename, db_filename)
